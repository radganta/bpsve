...
locals {
	...
	GCP_MGMT_FIREWALL_RULE_NAME = "management-firewall-rule-addon"
	...
	GCP_CONSOLE_FIREWALL_RULE_NAME = "gcp-console-firewall-rule-addon"
	...
	GCP_CONTROL_FIREWALL_RULE_NAME = "control-firewall-rule-addon"
	...
	GCP_TEST1_FIREWALL_RULE_NAME = "test-01-firewall-rule-addon"
	...
	GCP_TEST2_FIREWALL_RULE_NAME = "test-02-firewall-rule-addon"
	...
	GCP_VCONTROLLER_INSTANCE_NAME = "bps-vcontroller-01-addon"
	...
	GCP_VCONTROLLER_CUSTOM_IMAGE_FAMILY_NAME = "ixia-breakingpoint-virtual-controller-9-20-115"
	GCP_VCONTROLLER_MACHINE_TYPE = var.GCP_VCONTROLLER_MACHINE_TYPE
	GCP_VCONTROLLER_IFACE_ETH0_PUBLIC_IP_ADDRESS_NAME = "bps-vctrl-01-eth0-ip-addr-addon"
	GCP_VCONTROLLER_NETWORK_TARGET_TAGS = [
		"bps-vcontroller"
	]
	GCP_VBLADE_INSTANCE_NAME = "bps-vblade-01-addon"
	GCP_VBLADE_SERIAL_PORT_ENABLE = "true"
	GCP_VBLADE_CAN_IP_FORWARD = "true"
	GCP_VBLADE_CUSTOM_IMAGE_PROJECT_NAME = "bpsve-gcp"
	GCP_VBLADE_CUSTOM_IMAGE_FAMILY_NAME = "ixia-breakingpoint-virtual-blade-9-20-115"
	GCP_VBLADE_MACHINE_TYPE = var.GCP_VBLADE_MACHINE_TYPE
	GCP_VBLADE_IFACE_ETH0_PRIVATE_IP_ADDRESS = "10.0.10.13"
	GCP_VBLADE_IFACE_ETH0_PUBLIC_IP_ADDRESS_NAME = "bps-vblade-01-eth0-ip-addr-addon"
	GCP_VBLADE_IFACE_ETH1_PRIVATE_IP_ADDRESS = "10.0.2.112"
	GCP_VBLADE_IFACE_ETH1_PRIVATE_IP_ALIASES = "/28"
	GCP_VBLADE_IFACE_ETH2_PRIVATE_IP_ADDRESS = "10.0.3.112"
...
resource "google_compute_firewall" "GCP_MGMT_FIREWALL_RULE" {
	...
	disabled = "false"
	network = "${local.GCP_LOGIN_ID_TAG}-${local.GCP_PROJECT_TAG}-${local.GCP_MGMT_VPC_NETWORK_NAME}"
...
resource "google_compute_firewall" "GCP_TEST1_FIREWALL_RULE" {
	...
	disabled = "false"
	network = "${local.GCP_LOGIN_ID_TAG}-${local.GCP_PROJECT_TAG}-${local.GCP_TEST1_VPC_NETWORK_NAME}"
...
resource "google_compute_firewall" "GCP_TEST2_FIREWALL_RULE" {
	...
	disabled = "false"
	network = "${local.GCP_LOGIN_ID_TAG}-${local.GCP_PROJECT_TAG}-${local.GCP_TEST2_VPC_NETWORK_NAME}"
...
resource "google_compute_instance" "GCP_VCONTROLLER_INSTANCE" {
	...
	network_interface {
		network = "${local.GCP_LOGIN_ID_TAG}-${local.GCP_PROJECT_TAG}-${local.GCP_MGMT_VPC_NETWORK_NAME}"
		subnetwork = "${local.GCP_LOGIN_ID_TAG}-${local.GCP_PROJECT_TAG}-${local.GCP_MGMT_SUBNET_NAME}"
		access_config {
...
resource "google_compute_instance" "GCP_VBLADE_INSTANCE" {
	...
	network_interface {
		...
		subnetwork = "${local.GCP_LOGIN_ID_TAG}-${local.GCP_PROJECT_TAG}-${local.GCP_TEST1_SUBNET_NAME}"
		access_config {
			network_tier = "PREMIUM"
		}
		alias_ip_range {
	...
	network_interface {
		...
		subnetwork = "${local.GCP_LOGIN_ID_TAG}-${local.GCP_PROJECT_TAG}-${local.GCP_TEST2_SUBNET_NAME}"
		access_config {
			network_tier = "PREMIUM"
		}
		alias_ip_range {
